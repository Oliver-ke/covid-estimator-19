...
const computeCasesAndDollarFlight = (infectionsByRequestedTime,
  avgDailyIncomeInUSD, duration, avgDailyIncomePopulation) => {
  const casesForICUByRequestedTime = parseInt(infectionsByRequestedTime * (5 / 100), 10);
  const casesForVentilatorsByRequestedTime = parseInt(infectionsByRequestedTime * (2 / 100), 10);
  const dollarValue = (infectionsByRequestedTime * avgDailyIncomePopulation
  * avgDailyIncomeInUSD) / duration;
...
const computeSCandHB = (infectionsByRequestedTime, totalBeds) => {
  const severeCasesByRequestedTime = parseInt(infectionsByRequestedTime * (15 / 100), 10);
  const availableBeds = parseInt(totalBeds * (35 / 100), 10);
  let hospitalBedsByRequestedTime;
  if (severeCasesByRequestedTime > availableBeds) {
    hospitalBedsByRequestedTime = parseInt(availableBeds - severeCasesByRequestedTime, 10) + 1;
  } else {
    hospitalBedsByRequestedTime = parseInt(availableBeds, 10) + 1;
  }
  return {
...
const covid19ImpactEstimator = (data) => {
  const {
    ...
    totalHospitalBeds, region: { avgDailyIncomeInUSD, avgDailyIncomePopulation }
  } = data;
  ...
  const casesAndDollarsFlightImplact = computeCasesAndDollarFlight(
    impact.infectionsByRequestedTime, avgDailyIncomeInUSD, duration, avgDailyIncomePopulation
  );

  const casesAndDollarsFlightSevere = computeCasesAndDollarFlight(
    severeImpact.infectionsByRequestedTime, avgDailyIncomeInUSD, duration, avgDailyIncomePopulation
  );
